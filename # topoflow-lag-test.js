$(document).ready(function(){
    ...
    /**
        ...
        @todo: move away from topoflow.js
    */
    ...
    */
    ...
    let svg = svg_container.append("g").attr('class', 'main_group');
    ...
    */
    function calculate_offsets(number_of_links){
        ...
        */
        let data = [];
        let half_link_offset = options.link.offset/2;
        let lowest_offset = (half_link_offset*number_of_links-half_link_offset)*-1;
        for (i = lowest_offset; i <= lowest_offset*-1; i += options.link.offset){
            data.push(i);
        }
        return data;
    }
    ...
    */
    function draw_link(args){
        try{
            ...
            */
            let angle_a_to_b = Math.atan2(main_dataset.nodes[args.to].y - main_dataset.nodes[args.from].y, main_dataset.nodes[args.to].x - main_dataset.nodes[args.from].x);
            ...
            */
            ...
            let to_node_pos_x = main_dataset.nodes[args.to].x - offset_x;
            let to_node_pos_y = main_dataset.nodes[args.to].y + offset_y;
            let from_node_pos_x = main_dataset.nodes[args.from].x - offset_x;
            let from_node_pos_y = main_dataset.nodes[args.from].y + offset_y;
    ...
    function draw_link_1way(args){
        try{
            ...
            */
            ...
            let to_node_pos_x = (main_dataset.nodes[args.to].x - offset_x) - cos_to_angle * (options.node_radius + arrow_offset);
            let to_node_pos_y = (main_dataset.nodes[args.to].y + offset_y) - sin_to_angle * (options.node_radius + arrow_offset);
            let from_node_pos_x = main_dataset.nodes[args.from].x - offset_x;
            let from_node_pos_y = main_dataset.nodes[args.from].y + offset_y;
    ...
    */
    ...
    /*
        Do the magic!
    */
    run('this string will be substituted with a json file path in the future');
